"""
Bootstrap the local project so `include("src/SemioticTransformer.jl")` works even when
the repository has not been activated yet. Set `SEMIOTIC_BOOTSTRAP=0` to skip automatic
